...
#mectras <- mectras[str_detect(mectras, 'm2022|m2023')==T] # me quedo solo con ultimas mectras
...
for(i in 1:(length(mectras)-1)){
  ...
  if (i >= 93) { # Los casos de modalidad 102 empiezan a aparecer en septiembre del 2014, que es la mectra 90
...
# Ajusto los puestos de los meses falidos

# Calculo las variaciones interanuales
data_prov_letra_clae2 <- data_prov_letra_clae2[,var_inter := (puestos_totales/lag(puestos_totales, n=12)-1), by= c('Grupo', 'provincia', 'clae2')]
...
data_prov_letra_clae2 <- data_prov_letra_clae2[,variable2 := round(lag(puestos_totales, n=12)*(var_prom+1)), by=c('Grupo', 'provincia', 'clae2')]
# Ahora resta reemplazar los puestos en los meses fallados
data_prov_letra_clae2 <- data_prov_letra_clae2[,variable3 := fifelse((mes == '2009-06-01' | mes == '2009-10-01' | mes == '2012-12-01'), variable2, puestos_totales)]
data_prov_letra_clae2 <- data_prov_letra_clae2 %>% select( -c(puestos_totales, variable2, var_inter, var_prom))
setnames(data_prov_letra_clae2, 'variable3', 'puestos_totales')
...
data_total <- data_total[, mujeres := fifelse(is.na(mujeres), 0, mujeres)] # Le pongo a todas las mujeres que tengan NA cero
data_total <- data_total[, mujeres := fifelse(mujeres == 'Inf', 0, mujeres)]
# luego elimino los casos donde puestos sea NA
data_total <- data_total[!is.na(puestos)]
